...
class MyFlaskApp(Flask):
    def run(self, host=None, port=None, debug=None, **options):
        with open("bucket_midp/postings_gcp/index.pkl", 'rb') as f:
            self.index_body = pickle.loads(f.read())
        with open("bucket_midp/index_anchor.pkl", 'rb') as f:
            self.index_anchor = pickle.loads(f.read())
        with open("bucket_midp/index_title.pkl", 'rb') as f:
            self.index_title = pickle.loads(f.read())
        with open("bucket_midp/pageview/pageviews-202108-user.pkl", 'rb') as f:
            self.pageview = dict(pickle.loads(f.read()))
        self.df = pd.read_csv('bucket_midp/wikidumps/pr.csv', names=["id", "pagerank"])
        self.titles = pd.read_csv('bucket_midp/wikidumps/titles.csv', encoding="ISO-8859-1")
...
def search():
    ...
    for doc in body:
        ...
        if len(val) == 0:
            pagerank[doc[0]] = (doc[0], doc[1], 0.2 * doc[2])
        else:
            pagerank[doc[0]] = (doc[0], doc[1], 2 * (doc[2] * val[0]) / (doc[2] + val[0]))
...
def pg_table():
    # return dataframe table of all the docs
...
def search_by_body_index(query_to_search, N=-1):
    def get_candidate_documents_and_scores(query_to_search, index, N):
        """
        ...
        candidates = {}
        avg_text_len = 23824
        L = len(app.titles.index)
        for term in np.unique(query_to_search):
            list_of_doc = index.search(term)
            if not list_of_doc:
                continue
            normlized_tfidf = [(doc_id,
                                (freq / avg_text_len) * math.log(
                                    L / index.df[term], 10))
                               for
                               doc_id, freq in list_of_doc if doc_id in app.titles["id"]]

            for doc_id, tfidf in normlized_tfidf:
                candidates[(doc_id, term)] = candidates.get((doc_id, term), 0) + tfidf
        if N != -1:
            candidates = dict(sorted(candidates.items(), key=lambda item: item[1], reverse=True)[:N])
        return candidates
    ...
    def generate_document_tfidf_matrix(query_to_search, index, N):
        """
    ...
    def get_topN_score_for_query(query_to_search, index, N=-1):
        ...
        D = generate_document_tfidf_matrix(val, index, N)
        ...
        if N != -1:
            result = result[:N]
        result = [(id, app.titles.loc[app.titles["id"] == id]["title"].values[0], score) for id, score in result if
                  len(app.titles.loc[app.titles["id"] == id]["title"].values) != 0]
        return result
    ...
    def cosine_similarity(D, Q):
        ...
        i = 0
        for lst in tfidf:
            dic[index[i]] = np.dot(lst, Q) / (np.linalg.norm(lst) * np.linalg.norm(Q))
            i += 1
